...
__device__  __forceinline__ int64_t ld_volatile_global(const int64_t *ptr) {
    int64_t ret;
    ret = volatile_load((long long *)ptr);
...
__device__  __forceinline__ int64_t ld_volatile_global(const uint64_t *ptr) {
    int64_t ret;
    ret = volatile_load((unsigned long *)ptr);
    // asm volatile("ld.volatile.global.u64 %0, [%1];" : "=l"(ret) : "l"(ptr));
    return ret;
}

#ifndef DISABLE_AGGRESSIVE_MTX_INSTRS
...
barrier_device(int **task_fifo_ptrs, int head, int rank, int tag = 0) {
    ...
    if (thread_id < kNumRanks) {
        atomicAdd(task_fifo_ptrs[rank] + head + thread_id, FINISHED_SUM_TAG);
        memory_fence();
        atomicAdd(task_fifo_ptrs[thread_id] + head + rank, -FINISHED_SUM_TAG);
    }
